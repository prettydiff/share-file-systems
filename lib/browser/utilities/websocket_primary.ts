...
/* lib/browser/utilities/websocket_primary - Handles web socket events and associated errors. This where most communications from outside the browser are processed. */
...
const websocket_primary = function browser_utilities_websocketPrimary(callback:() => void, hashDevice:string):void {
    const open = function browser_utilities_webSocket_socketOpen():void {
        const title:HTMLElement = document.getElementById("title-bar");
            if (title.getAttribute("class") === "title offline") {
                location.reload();
            } else {
                title.setAttribute("class", "title");
                const socketError = function browser_utilities_socketError():void {
                        setTimeout(function browser_utilities_socketError_delay():void {
                            browser_utilities_websocketPrimary(callback, hashDevice);
                        }, browser.ui.statusTime);
                    },
                    messageDelay = function browser_utilities_webSocket_socketOpen_messageDelay():void {
                    ...
                    close = function browser_utilities_webSocket_socketOpen_socketClose():void {
                        if (browser.identity.hashDevice !== "" && socket.type === "primary") {
                            const device:HTMLElement = document.getElementById(browser.identity.hashDevice),
                                ...
                                title:HTMLElement = document.getElementById("title-bar"),
                                h1:HTMLElement = title.getElementsByTagName("h1")[0];
                            ...
                            h1.empty();
                            h1.appendText("Disconnected.");
                            if (device !== null) {
                                device.setAttribute("class", "offline");
                            }
                        }
                    },
                    receiver = function browser_utilities_webSocket_socketOpen_receiver(event:websocket_event):void {
                        const dataString:string = (typeof event.data === "string")
                                ? event.data
                                : null,
                            error = function browser_utilities_webSocket_socketOpen_receiver_error():void {
                                // eslint-disable-next-line no-console
                                console.error("Error", socketData.data);
                            },
                            reload = function browser_utilities_webSocket_socketOpen_receiver_reload():void {
                                location.reload();
                            },
                            actions:network_actions = {
                                "agent-hash": function browser_utilities_webSocket_socketOpen_receiver_agentHash(socketData:socketData):void {
                                    if (browser.identity.hashDevice === "") {
                                        ...
                                        agent_add({
                                            callback: function browser_utilities_webSocket_socketOpen_receiver_agentHash_addAgent():void {
                                ...
                                "agent-status": function browser_utilities_webSocket_socketOpen_receiver_agentStatus(socketData:socketData):void {
                                ...
                                "agent-management": function browser_utilities_webSocket_socketOpen_receiver_agentManagement(socketData:socketData):void {
                                    const data:service_agentManagement = socketData.data as service_agentManagement;
                                    if (data.action === "add") {
                                        const addAgents = function browser_utilities_webSocket_socketOpen_receiver_agentManagement_addAgents(agentType:agentType):void {
                                        ...
                                        const deleteAgents = function browser_utilities_webSocket_socketOpen_receiver_agentManagement_deleteAgents(agentType:agentType):void {
                                            const keys:string[] = Object.keys(data.agents[agentType]),
                                                keyLength:number = keys.length,
                                        ...
                                        const shareContent = function browser_utilities_webSocket_socketOpen_receiver_agentManagement_shareContent(agentName:string, agentType:agentType|""):void {
                                            ...
                                            modifyAgents = function browser_utilities_webSocket_socketOpen_receiver_agentManagement_modifyAgents(agentType:agentType):void {
                                                const keys:string[] = Object.keys(data.agents[agentType]),
                                                    keyLength:number = keys.length;
                                                if (keyLength > 0) {
                                                    let a:number = 0;
                                ...
                                "error": error,
                                "hash-share": function browser_utilities_webSocket_socketOpen_receiver_hashShare(socketData:socketData):void {
                                ...
                                "file-system-details": function browser_utilities_webSocket_socketOpen_receiver_fileDetails(socketData:socketData):void {
                                    const payload:service_fileSystem_details = socketData.data as service_fileSystem_details,
                                        ...
                                        row = function browser_utilities_webSocket_socketOpen_receiver_fileDetails_row(heading:string, cell:string, tbody:HTMLElement):void {
                                    ...
                                    if (list[0][1] === "directory" && details.files > 0) {
                                        const dataLength:number = fileList.length,
                                            ...
                                            sortAlpha = function browser_utilities_webSocket_socketOpen_receiver_fileDetails_sortAlpha(aa:directory_item, bb:directory_item):number {
                                                if (aa[0] < bb[0]) {
                                                    return -1;
                                                }
                                                return 1;
                                            },
                                            sortChanged = function browser_utilities_webSocket_socketOpen_receiver_fileDetails_sortChanged(aa:directory_item, bb:directory_item):number {
                                                if (aa[5].mtimeMs > bb[5].mtimeMs) {
                                                    return -1;
                                                }
                                                return 1;
                                            },
                                            sortLargest = function browser_utilities_webSocket_socketOpen_receiver_fileDetails_sortLargest(aa:directory_item, bb:directory_item):number {
                                                if (aa[5].size > bb[5].size) {
                                                    return -1;
                                                }
                                                return 1;
                                            },
                                            clickGenerator = function browser_utilities_webSocket_socketOpen_receiver_fileDetails_clickGenerator(sortName:"alpha"|"changed"|"largest"):void {
                                                ...
                                                button.onclick = function browser_utilities_webSocket_socketOpen_receiver_fileDetails_clickGenerator_click(event:MouseEvent):void {
                                                    if (sortName === "alpha") {
                                ...
                                "file-system-string": function browser_utilities_webSocket_socketOpen_receiver_fileString(socketData:socketData):void {
                                ...
                                "message": function browser_utilities_webSocket_socketOpen_receiver_message(socketData:socketData):void {
                                    ...
                                    messageData.forEach(function browser_utilities_webSocket_socketOpen_receiver_message_each(item:message_item):void {
                                    ...
                                    if (browser.visible === false && Notification.permission === "granted") {
                                        ...
                                        notify.onshow = function browser_utilities_webSocket_socketOpen_receiver_message_show():void {
                                            notify.close();
                                        };
                                    }
                                },
        ...
        socket:websocket_browser = websocket_open(open, "primary", hashDevice);
    browser.socket = socket;
};

export default websocket_primary;