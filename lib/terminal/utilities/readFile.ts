...
const readFile = function terminal_utilities_readFile(args:readFile):void {
    ...
    vars.node.fs.open(args.path, "r", function terminal_utilities_readFile_open(ero:Error, fd:number):void {
        const messageSize:number = (args.stat.size < 100)
            ? args.stat.size
            : 100;
        let buff  = Buffer.alloc(Number(messageSize));
        if (ero === null) {
            vars.node.fs.read(fd, buff, 0, messageSize, 1, function terminal_utilities_readFile_open_read(errA:Error, bytesA:number, bufferA:Buffer):number {
                ...
                if (errA === null) {
                    ...
                    if (vars.binary_check.test(bufferString) === true) {
                        ...
                        vars.node.fs.read(fd, buff, 0, args.stat.size, 0, function terminal_utilities_readFile_open_read_readBinary(errB:Error, bytesB:number, bufferB:Buffer):void {
                            if (errB === null) {
                                if (bytesB > 0) {
                        ...
                        vars.node.fs.readFile(args.path, {
                            encoding: "utf8"
