        ...
        const order = {
            ...
            phases:buildPhaseList = {
                ...
                clearStorage: function terminal_commands_build_clearStorage():void {
                    ...
                    vars.node.fs.readdir(`${vars.projectPath}lib${vars.sep}settings`, function terminal_commands_build_clearStorage_dir(erd:Error, dirList:string[]) {
                        if (erd !== null) {
                            error([erd.toString()]);
                            return;
                        }
                        ...
                        do {
                            if (tempTest.test(dirList[a]) === true) {
                                start = start + 1;
                                vars.node.fs.unlink(`${vars.projectPath}lib${vars.sep}settings${vars.sep + dirList[a]}`, function terminal_commands_build_clearStorage_dir_unlink(eru:Error):void {
                                    if (eru !== null) {
                                        error([erd.toString()]);
                                        return;
                                    }
                            ...
                            const globalName:string = "share",
                                ...
                                files = function terminal_commands_build_shellGlobal_npm_files():void {
                                    ...
                                    if (windows === true) {
                                        ...
                                        const cyg:string = `#!/bin/sh\nbasedir=$(dirname "$(echo "$0" | sed -e 's,\\\\,/,g')")\n\ncase \`uname\` in\n    *CYGWIN*|*MINGW*|*MSYS*) basedir=\`cygpath -w "$basedir"\`;;\nesac\n\nif [ -x "$basedir/node" ]; then\n  exec "$basedir/node"  "$basedir/node_modules/${globalName}/bin/${globalName}" "$@"\nelse\n  exec node  "$basedir/node_modules/${globalName}/bin/${globalName}" "$@"\nfi\n`,
                                            cmd:string = `@ECHO off\r\nGOTO start\r\n:find_dp0\r\nSET dp0=%~dp0\r\nEXIT /b\r\n:start\r\nSETLOCAL\r\nCALL :find_dp0\r\n\r\nIF EXIST "%dp0%\\node.exe" (\r\n  SET "_prog=%dp0%\\node.exe"\r\n) ELSE (\r\n  SET "_prog=node"\r\n  SET PATHEXT=%PATHEXT:;.JS;=;%\r\n)\r\n\r\nendLocal & goto #_undefined_# 2>NUL || title %COMSPEC% & "%_prog%"  "%dp0%\\node_modules\\${globalName}\\bin\\${globalName}" %*\r\n`,
                                            ps1:string = `#!/usr/bin/env pwsh\n$basedir=Split-Path $MyInvocation.MyCommand.Definition -Parent\n\n$exe=""\nif ($PSVersionTable.PSVersion -lt "6.0" -or $IsWindows) {\n  $exe=".exe"\n}\n$ret=0\nif (Test-Path "$basedir/node$exe") {\n  if ($MyInvocation.ExpectingInput) {\n    $input | & "$basedir/node$exe"  "$basedir/node_modules/${globalName}/bin/${globalName}" $args\n  } else {\n    & "$basedir/node$exe"  "$basedir/node_modules/${globalName}/bin/${globalName}" $args\n  }\n  $ret=$LASTEXITCODE\n} else {\n  if ($MyInvocation.ExpectingInput) {\n    $input | & "node$exe"  "$basedir/node_modules/${globalName}/bin/${globalName}" $args\n  } else {\n    & "node$exe"  "$basedir/node_modules/${globalName}/bin/${globalName}" $args\n  }\n  $ret=$LASTEXITCODE\n}\nexit $ret\n`,
                                            // cspell:enable
