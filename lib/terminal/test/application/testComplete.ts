...
const testComplete = function terminal_test_application_testComplete(complete:test_complete):void {
    ...
    if (complete.failures > 0) {
        ...
        complete.callback(`${common.capitalize(complete.testType)} Test`, [`${vars.text.green}Successfully completed all ${vars.text.cyan + vars.text.bold + String(complete.total) + vars.text.none + vars.text.green} ${complete.testType} tests.${vars.text.none}`], false);
    }
};
