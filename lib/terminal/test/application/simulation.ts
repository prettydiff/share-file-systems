...
const simulation:module_test_simulationApplication = {
    execute: function terminal_test_application_simulations_execute(config:config_test_execute):void {
        ...
        node.child_process.exec(`${vars.terminal.command_instruction + command} test_simulation`, {cwd: vars.terminal.cwd, maxBuffer: 2048 * 500}, function terminal_test_application_simulations_execution_child(errs:node_childProcess_ExecException, stdout:string, stdError:Buffer | string) {
