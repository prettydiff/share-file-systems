...
const testMessage = function test_testMessage(input:testMessage) {
    if (input.messages[0] === "") {
        ...
        log([`${humanTime(false) + vars.text.angry}Fail ${input.testType} ${input.index + 1}: ${vars.text.none + name} ${vars.text.angry + input.messages[0].replace("fail - ", "") + vars.text.none}`]);
        if (input.messages[1] !== "") {
