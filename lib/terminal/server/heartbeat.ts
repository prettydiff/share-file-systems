...
const heartbeat:heartbeatObject = {
    ...
    complete: function terminal_server_heartbeat_complete(dataPackage:socketData, transmit:transmit, remoteIP:string):void {
    ...
    update: function terminal_server_heartbeat_update(dataPackage:socketData):void {
        ...
        if (serverVars.testType === "service" && dataPackage.service.indexOf("heartbeat") === 0) {
            responder({
                data: data,
                service: dataPackage.service
            }, {
